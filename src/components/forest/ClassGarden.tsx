  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-gray-800">우리 반 텃밭</h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {classmates.map((mate) => {
            ...
            if (!isMe && !hasVerified) {
              // 잠자는 씨앗 (미인증 타인)
              return (
                <div key={mate.uid} className="aspect-square bg-gray-50 rounded-2xl border border-gray-100 flex flex-col items-center justify-center p-4 opacity-50 cursor-not-allowed">
                  <Sprout className="text-gray-400 mb-2" size={32} />
                  <span className="text-sm text-gray-400 font-medium text-center">잠자는 씨앗</span>
                </div>
              );
            }

            // 인증했거나 나인 경우
            const treeColor = hasVerified ? "text-primary-olive" : "text-gray-400";
            const treeBg = hasVerified ? "bg-primary-olive/10" : "bg-gray-100";
            const level = mate.treeLevel || 1;

            return (
              <div
                key={mate.uid}
                className={`aspect-square ${treeBg} rounded-2xl border ${hasVerified ? "border-primary-olive/30" : "border-gray-200"} flex flex-col items-center justify-center p-4 transition-transform hover:scale-105`}
              >
                <div className={`relative ${treeColor} mb-2`}>
                  <TreePine size={48} />
                  <div className="absolute -bottom-2 -right-2 bg-white rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold text-gray-700 shadow-sm border border-gray-100">
                    Lv.{level}
                  </div>
                </div>
                ...
                {hasVerified && (
                  <span className="text-xs text-point-blue mt-1 font-medium bg-white px-2 py-0.5 rounded-full shadow-sm">
                    인증 완료!
                  </span>
                )}
                {isMe && !hasVerified && (
